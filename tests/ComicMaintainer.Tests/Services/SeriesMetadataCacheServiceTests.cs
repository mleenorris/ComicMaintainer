...
public class SeriesMetadataCacheServiceTests
{
    ...
    [Fact]
    public async Task SetSeriesNameAsync_UpsertsRecord_AsUserSelected()
    {
        var record = await _service.SetSeriesNameAsync("Berserk", "My Berserk");

        Assert.Equal("berserk", record.NormalizedKey);
        Assert.Equal("My Berserk", record.SeriesName);
        Assert.Equal(SeriesNameSource.UserSelected, record.SeriesNameSource);
        Assert.Null(record.SeriesNameLanguage);
    }

    [Fact]
    public async Task SetSeriesNameAsync_TrimsName()
    {
        var record = await _service.SetSeriesNameAsync("Berserk", "  Trimmed Name  ");
        Assert.Equal("Trimmed Name", record.SeriesName);
    }

    [Fact]
    public async Task SetSeriesNameAsync_IsSticky_AcrossRefresh()
    {
        await _service.SetSeriesNameAsync("Bleach", "User Pick");
        ...
        Assert.Equal("User Pick", refreshed.SeriesName);
        Assert.Equal(SeriesNameSource.UserSelected, refreshed.SeriesNameSource);
    }

    [Fact]
    public async Task SetSeriesNameAsync_Reset_RecomputesFromInputs()
    {
        _external.Setup(e => e.LookupSeriesAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ExternalSeriesMetadata
            {
                CanonicalTitle = "One Piece",
                LocalizedTitles = new List<LocalizedTitle> { new("One Piece", "en"), new("ワンピース", "ja") }
            });
        await _service.RefreshAsync("One Piece");
        await _service.SetPreferredLanguageAsync("One Piece", "ja");
        await _service.SetSeriesNameAsync("One Piece", "Override");

        var cleared = await _service.SetSeriesNameAsync("One Piece", null);

        Assert.NotEqual(SeriesNameSource.UserSelected, cleared.SeriesNameSource);
        Assert.Equal("ワンピース", cleared.SeriesName);
        Assert.Equal(SeriesNameSource.LanguageDefault, cleared.SeriesNameSource);
    }
    ...
    public async Task SetSeriesNameAsync_BumpsMetadataVersion()
    {
        _external.Setup(e => e.LookupSeriesAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            ...
            {
                CanonicalTitle = "Berserk",
                LocalizedTitles = new List<LocalizedTitle> { new("Berserk", "en") }
            });
        var initial = await _service.RefreshAsync("Berserk");
        var initialVersion = initial.MetadataVersion;

        var named = await _service.SetSeriesNameAsync("Berserk", "Renamed");
        Assert.True(named.MetadataVersion > initialVersion,
            $"Expected MetadataVersion to be bumped (was {initialVersion}, now {named.MetadataVersion}).");
    }

    [Fact]
    public async Task SetSeriesNameAsync_Throws_WhenSeriesTitleEmpty()
    {
        await Assert.ThrowsAsync<ArgumentException>(
            () => _service.SetSeriesNameAsync("", "Name"));
    }
}
