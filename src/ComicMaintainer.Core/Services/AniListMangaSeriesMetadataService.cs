...
/// <summary>
/// External series metadata provider for Manga (Japanese-origin comics) backed by
/// the public AniList GraphQL API. Results are filtered with
/// <c>countryOfOrigin: "JP"</c>, complementing the sibling AniList Manhwa
/// provider (KR) and the MangaDex / ComicVine providers in the composite chain.
/// </summary>
public class AniListMangaSeriesMetadataService : IExternalSeriesMetadataService
{
    ...
    private readonly ILogger<AniListMangaSeriesMetadataService> _logger;
    private readonly ProviderHealthTracker _health = new("AniListManga");
    private readonly Lazy<ProviderRateLimiter> _rateLimiter;

    public AniListMangaSeriesMetadataService(
        IHttpClientFactory httpClientFactory,
        IOptionsMonitor<AppSettings> settings,
        IMemoryCache cache,
        ILogger<AniListMangaSeriesMetadataService> logger)
    {
    ...
    public string ProviderName => "AniListManga";
    ...
    public async Task<ExternalSeriesMetadata?> LookupSeriesAsync(string seriesName, CancellationToken cancellationToken = default)
    {
        ...
        var cacheKey = $"anilist-manga-series:{Normalize(seriesName)}";
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            using var httpClient = _httpClientFactory.CreateClient(nameof(AniListMangaSeriesMetadataService));
            ...
            {
                _logger.LogWarning(
                    "AniList manga lookup failed for {SeriesName} with status code {StatusCode}",
        ...
        {
            _logger.LogWarning(ex, "AniList manga lookup failed for {SeriesName}", LoggingHelper.SanitizeForLog(seriesName));
    ...
    public async Task<ProviderHealth> CheckHealthAsync(CancellationToken cancellationToken = default)
    {
        ...
        const string probeCacheKey = "anilist-manga-health-probe";
