...
# Note: In-memory caching has been removed as SQLite database is fast enough
# (< 10ms for 5000 files). This simplifies the codebase significantly by
# removing cache invalidation logic, locks, and async rebuild complexity.

# Cache lock functions removed - no longer needed without in-memory caching

# Wrapper functions for marker operations - simplified without cache invalidation
...
class WatcherMonitorHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        if not event.is_directory and event.src_path.endswith(CACHE_UPDATE_MARKER):
            logging.info(f"Watcher activity detected via file system event")
            # No cache to update - data is always fresh from database
...
def record_file_change(change_type, old_path=None, new_path=None):
    ...
    try:
        ...
        # No cache to invalidate - data is always fresh from database
    except Exception as e:
...
def get_comic_files():
    ...
    # Load from file store database (fast - < 2ms for 1000 files)
    logging.debug("Loading file list from database")
...
def get_enriched_file_list(files):
    """Get file list enriched with metadata from database (no caching)
    ...
    """
    # Get all marker data in one batch query (fast - < 5ms for 5000 files)
    ...
    # Get all file metadata from database in a single query (fast - < 3ms for 5000 files)
    ...
    for f in files:
        ...
        # Get metadata from database (fast) or fall back to os.path (slow fallback)
...
def get_filtered_sorted_files(all_files, filter_mode, search_query, sort_mode, sort_direction):
    """Get filtered and sorted files (no caching - database is fast enough)
    ...
    """
    # Filtering and sorting is very fast (< 1ms for 5000 files), no caching needed
    logging.debug(f"Filtering files: filter={filter_mode}, search='{search_query}', sort={sort_mode}, direction={sort_direction}")
...
def list_files():
    ...
    # Get files directly from database (fast - < 10ms for 5000 files)
    files = get_comic_files()
    
    # Get enriched file list with metadata from database (fast - < 10ms for 5000 files)
    ...
    # Get filtered and sorted files (fast - < 1ms for 5000 files)
...
def cache_stats_endpoint():
    """API endpoint to get database statistics (caching removed)"""
    try:
        ...
        return jsonify({
            ...
            'message': 'In-memory caching has been removed - database queries are fast enough (< 10ms for 5000 files)',
            'database': {
            ...
            'markers': {
                ...
                'storage_location': '/Config/store/comicmaintainer.db'
            }
        })
    except Exception as e:
...
def init_app():
    ...
    if last_sync is None or (current_time - last_sync) > 300:
        logging.info("Syncing file store with filesystem on startup...")
    ...
    logging.info("Application initialized - all data is loaded directly from SQLite database (no caching needed)")
